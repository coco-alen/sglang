...
impl GrpcPDRouter {
    ...
    async fn route_generate_impl(
        ...
        if body.stream {
            return (
                ...
                "Streaming generate over gRPC PD mode is not supported yet",
            )
                .into_response();
        }
    ...
    async fn handle_streaming_chat(
        &self,
        mut _prefill_client: SglangSchedulerClient,
        mut _decode_client: SglangSchedulerClient,
    ...
    async fn handle_non_streaming_generate(
        ...
        let mut complete = match decode_responses.into_iter().next() {
            Some(c) => c,
            None => {
